...
export const ModalCadastroMedico = ()=> {
  const {
  ...
  const handleBlur = (estado, id, id2) => {
    if (
      ...
      crmClienteInput.replace(/\D/g, "").length < 11
    ) {
  ...
  const checkValues = () => {
    ...
    if (crmClienteInput.replace(/\D/g, "").length < 1) {
      setSucesso(false);
      setMensagem("CRM invalido");
      handleMostraMensagem();
      return;
    }
  ...
  async function handleCadastroCliente() {
    try {
      const response = await fetch(
        `${urlMedicos}/cadastrar`,
        {
          ...
          body: JSON.stringify({
            ...
            crm: crmClienteInput.toString().replace(/\D/g, "").trim(),
      ...
      if (data.sucesso) {
        setModalCadastroMedicoAberto(false);
